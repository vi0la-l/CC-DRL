...
feature_4ch = Dense(256, activation='swish')(feature_4ch)  # 增强特征交互
feature_4ch = BatchNormalization()(feature_4ch)          # 稳定训练
feature_4ch = Dropout(0.3)(feature_4ch)                 # 防止过拟合

feature_merge1 = AttentionWeightedAverage2(name='att1')(tf.expand_dims(feature_4ch, axis=1))
feature_merge2 = AttentionWeightedAverage2(name='att2')(tf.expand_dims(feature_4ch, axis=1))
...
def autoencoder_loss(y_true, y_pred):
    ...
    cost_matrix = tf.constant(np.loadtxt("BSL_Confusion_Matrix.txt", delimiter='\t'), dtype=tf.float32)
    tf.linalg.set_diag(cost_matrix, tf.zeros(cost_matrix.shape[0]))  # 保证对角线为0

    indices = tf.stack([true_class_indices, pred_class_indices], axis=1)
    misclass_cost = tf.gather_nd(cost_matrix, indices)

    return ce + 0.5 * misclass_cost  # 可调节惩罚力度
